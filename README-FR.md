...
- **Courant maximal** (curseur numérique, A) — OCPP (smart-charging `SetChargingProfile`), Wallbox, AVE, Sigenergy AC. Les limites viennent du serveur (typiquement 6 A jusqu'au maximum de la borne). Non disponible pour Sigenergy DC (aucune API de limite de courant côté DC).
...
- **La force du signal WiFi n'est disponible pour aucune borne, chez aucun vendeur.** L'appareil de Tesla rapporte bien une valeur de force du signal, mais seulement via son API réseau locale sur le même LAN — l'API infonuagique que lit cette plateforme ne la transporte pas, il n'y a donc aucun moyen de l'exposer ici pour Tesla ou tout autre vendeur.
